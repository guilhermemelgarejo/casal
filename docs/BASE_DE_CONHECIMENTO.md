...
- **store:** `funding` = `account` \| `credit_card`; categoria e conta do casal; tipo da categoria = tipo do lançamento; rejeita categoria com `isCreditCardInvoicePayment()` (quitação de fatura). Na UI, o cadastro abre em **modal** Bootstrap (botão “Novo lançamento” na listagem); o utilizador escolhe primeiro a **forma de pagamento** (incl. “Cartão de crédito”); em seguida o **cartão** ou as **contas** que aceitam aquela forma (`resources/views/transactions/index.blade.php` + `public/js/app.js`). Com erro de validação no `store`, a modal reabre com `old()`; sem contas, a modal mostra aviso e atalho para Contas.
