...
- **update:** `PUT /transactions/{transaction}` — altera **apenas** `amount`; **`transaction_category_splits`** são **reescaladas** na mesma proporção (cada parcela de um parcelamento é independente). Corpo opcional **`return_from_installment_modal`** (`1` quando a edição veio da modal de parcelas); em sucesso (ou valor inalterado), se o lançamento pertencer a um grupo com mais de uma parcela, flash **`open_installment_modal_root`** para a página de lançamentos reabrir a modal **Parcelas da compra** com dados atualizados. **Bloqueado:** lançamento com **`creditCardStatementsPaidFor`** (pagamento de fatura); despesa no **cartão** se o ciclo (`account_id` + referência) tiver **`CreditCardStatement::blocksEditingCardExpenses()`** (qualquer pagamento vinculado ou fatura quitada). Sem splits por categoria, rejeita. Despesa no cartão com **`tracksCreditCardLimit()`**: ao **aumentar** o valor, o fluxo em **`public/js/app.js`** chama **`POST /transactions/{transaction}/credit-limit-precheck-update`** (`transactions.credit-limit-precheck-update`); sessão **`credit_limit_overflow_pending_tx_update`** + **`credit_limit_confirm_token`** no **update** (paralelo ao **store**). **UI:** ícone de edição na lista em `resources/views/transactions/index.blade.php`; modal “Alterar valor” (botão **Salvar**); erros de validação reabrem o modal certo (flash `edit_transaction_id` evita abrir o modal de “Novo lançamento”).

### `CategoryController`

- CRUD com `abort(403)` se recurso de outro casal. Categoria com `isCreditCardInvoicePayment()` **não** pode ser editada nem excluída; **`store` / `update`** também impedem `name` igual a `Category::NAME_CREDIT_CARD_INVOICE_PAYMENT`. **`index`:** a partir de `md`, duas colunas — formulário à esquerda; à direita, título “Suas categorias” e dois cartões empilhados (receitas e despesas), ordenados por nome. Formulário compacto (borda leve, campos `sm`). UI: “Fixa” em `resources/views/categories/index.blade.php`.
...
- Rotas sob **`/faturas-cartao`** (`credit-card-statements.index`, `update`, `attach-payment`), middleware `auth` + `has-couple` + `couple-billing`. **index:** query opcional **`account_id`** (GET) — filtra por cartão do casal; valor inválido ou de outro tipo de conta é ignorado (lista todos). Agrupa **despesas** por (`account_id` cartão, `reference_month`, `reference_year`); coluna de total usa **`spent_total`** do merge com `credit_card_statements` quando existe metadado, senão a soma agregada das transações; **vencimento “Sug.”** quando não há `due_date` gravado mas o cartão tem dia configurado (`defaultStatementDueDate`). Coluna **Ações:** ordem dos botões **Pagamento** (quando a UI permite novo pagamento), **Editar**, **Itens da fatura**. Cada linha da tabela de faturas tem `id` estável `statement-cycle-{account_id}-{reference_year}-{reference_month}` para âncora ao vir da modal de parcelas em Lançamentos. Botão **Itens da fatura** abre modal com cada **lançamento daquele ciclo** (uma linha por parcela), colunas: data da compra, descrição, parcela (ex. `1/3`), ref. da fatura, **valor só desta fatura**, link **Abrir** para Lançamentos com filtro pelo **mês civil da data da compra** da parcela e pelo cartão (alinhado à listagem de despesas no cartão). Os itens vêm de `invoiceCycleLinesByKey` (JSON em `window.__invoiceCycleLinesByKey` na página), chave `account_id-reference_year-reference_month` — evita dados grandes em `data-*` no botão. **Pagamento** abre em **modal** Bootstrap (mesmo padrão que “Editar”); em erro de validação, `open_statement_payment` reabre a modal com `old()`. Pagamento só por **criar** lançamento na conta corrente (sem vincular lançamento existente).
