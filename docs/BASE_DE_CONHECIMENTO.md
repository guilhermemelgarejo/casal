...
- CRUD com `abort(403)` se recurso de outro casal. **`store` (cartão):** `credit_card_invoice_due_day` opcional (1–31); se omitido, **10**. **`update` (cartão):** mesmo campo opcional; vazio = sem sugestão de vencimento. Conta `regular`: `credit_card_invoice_due_day` gravado como `null`. UI em `resources/views/accounts/index.blade.php` (badges de formas de pagamento na conta = lista canónica; campo de dia só para cartão). **`kind` da conta é fixo após criação** (não editável na UI; `update` não altera `kind` mesmo com parâmetros na requisição).

### `CreditCardStatementController`

- Rotas sob **`/faturas-cartao`** (`credit-card-statements.*`), middleware `auth` + `has-couple` + `couple-billing`. **index:** agrupa **despesas** por (`account_id` cartão, `reference_month`, `reference_year`); coluna de total usa **`spent_total`** do merge com `credit_card_statements` quando existe metadado, senão a soma agregada das transações; **vencimento “Sug.”** quando não há `due_date` gravado mas o cartão tem dia configurado (`defaultStatementDueDate`); lançamentos elegíveis para vincular como pagamento.
...
- Pastas: `tests/Feature` (Auth, Profile, CoupleAccess, CategoryCrud, `CreditCardStatementTest`, `CreditCardInvoicePaymentExcludedFromStatisticsTest`, `CreditCardInvoiceCategoryExclusionTest`, …), `tests/Unit` (ex.: `PaymentMethodsTest`, `AccountDefaultStatementDueDateTest`).
