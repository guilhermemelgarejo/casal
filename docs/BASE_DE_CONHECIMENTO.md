...
- **Totais** (receita/despesa/saldo), **alerta** de gastos e bloco **“Onde vocês gastaram”** usam transações com `excludingCreditCardInvoicePayments()` (pagamentos de fatura de cartão **fora** desses números). Esse bloco lista **despesas por conta** (total no período), com **etiqueta de tipo** (Conta vs Cartão de crédito), **sem** detalhe por forma de pagamento.
