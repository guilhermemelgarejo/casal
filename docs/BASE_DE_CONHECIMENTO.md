...
- Query param `period=YYYY-MM` (default mês atual): o **mês e ano extraídos** alimentam os **KPIs** (receita/despesa/saldo e alerta) pelo **mês de referência** (`reference_month`/`reference_year`).
- **KPIs** e **alerta** usam `whereMatchesDashboardKpiPeriod`: **receitas** no mês; **despesas** só **em caixa** — exclui compras no **cartão de crédito**; **inclui** pagamentos de fatura (conta corrente vinculada à fatura) e despesas em conta **regular** (débito, Pix, etc.).
...
- **`store`:** rejeita categoria `isCreditCardInvoicePayment()` (redirect com fragmento e `withInput`). Listagens em query usam `excludingCreditCardInvoicePayment()`; orçamentos do mês com `whereHas` na mesma lógica. Sucesso e `updateIncome` redirecionam para `categories.index` com `withFragment('orcamento')`.
