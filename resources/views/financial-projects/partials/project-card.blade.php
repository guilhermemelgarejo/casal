        ...
        <div class="d-flex align-items-center gap-2 pt-1 mt-auto">
            ...
            @elseif($isAsset)
                ...
                <button
                    ...
                    data-bs-toggle="modal"
                    data-bs-target="#modalCofrinhoAssetVenda{{ $p->id }}"
                    title="Registrar venda / resgate deste ativo"
                >− Venda {{ $p->isBitcoin() ? 'BTC' : $p->assetUnitLabel() }}</button>
            @else
                <button
                    type="button"
                    class="btn btn-success btn-sm rounded-pill px-3 flex-grow-1 fw-bold text-white shadow-sm"
...
@if($isAsset)
    ...
    {{-- Modal Venda em Ativo (se for ativo) --}}
    <div
        class="modal fade"
        id="modalCofrinhoAssetVenda{{ $p->id }}"
        tabindex="-1"
        aria-labelledby="modalCofrinhoAssetVendaLabel{{ $p->id }}"
        aria-hidden="true"
    >
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                <div class="modal-header bg-danger text-white border-0">
                    <h2 class="modal-title h5 mb-0 text-white" id="modalCofrinhoAssetVendaLabel{{ $p->id }}">Venda / Resgate — {{ $p->name }}</h2>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <form method="post" action="{{ route('cofrinhos.asset-sale.store', $p) }}" class="js-asset-venda-form" data-cofrinho-id="{{ $p->id }}">
                    @csrf
                    <div class="modal-body vstack gap-3">
                        {{-- Posicao Atual --}}
                        <div class="p-3 rounded-3 border border-secondary-subtle bg-body-secondary">
                            <div class="row g-2 text-center">
                                ...
                                <div class="col-6">
                                    <span class="small text-secondary d-block">Preço Médio atual</span>
                                    <strong class="fs-6">R$ {{ number_format((float) $p->asset_avg_price, 2, ',', '.') }}</strong>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-3 rounded-3 border border-secondary-subtle bg-body-tertiary">
                            ...
                            <div class="js-account-tx-fields vstack gap-2 pt-2 border-top border-secondary-subtle">
                                ...
                                <div>
                                    <x-input-label for="asset_venda_date_{{ $p->id }}" value="Data da transação" />
